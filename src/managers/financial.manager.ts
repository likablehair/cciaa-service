...
export class FinancialManager {
  async getFinancialValues(
    ...
    if (!base64Xbrl) {
      pushAIWSError(errors, AIWS_ERROR_CODE.XBRL_EMPTY, {}, AIWS_ERROR_CODE['XBRL_EMPTY'])
      return null
    }
    ...
    try {
      ...
      pushAIWSError(errors, AIWS_ERROR_CODE.XBRL_DECODE_ERROR, {
        error: String(err),
    ...
    try {
      ...
      pushAIWSError(errors, AIWS_ERROR_CODE.XML_PARSE_ERROR, {
        error: String(err),
    ...
    if (!xbrlJson?.xbrl) {
      pushAIWSError(errors, AIWS_ERROR_CODE.XBRL_MAPPING_ERROR, {}, AIWS_ERROR_CODE['XBRL_MAPPING_ERROR'])
      return null
    }
