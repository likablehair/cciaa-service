...
export class PersonService extends BaseService {
  ...
  public async getPersonCorporateRoles(params: {
    ...
    try {
      ...
      if (!this.checkResponseStatus(response.status, params.errors)) return null;

      const json = this.parseXml<ParsedAIWSResponse<PersonaData>>(response.data, params.errors);
      if (!json) return null;
